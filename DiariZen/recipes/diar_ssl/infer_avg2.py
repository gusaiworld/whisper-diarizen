    ...
    if args.uem_file:
        ...
        for sess in sess2audio.keys():
            ...
            with open(c_rttm_out, "w") as f:
                print(c_result.to_rttm()[0])
                f.write(c_result.to_rttm())

            with open(c_rttm_out, "r") as f:
                    lines = f.readlines()
            txt_c = ''.join(lines)
            path = sess
            reg_spk = r"(?<=<NA> <NA> ).+?(?= <NA>)"
            reg = r"(?<=%s 1 ).+?(?= <NA>) " % path  # 动态正则

            spkid = re.findall(reg_spk, txt_c)
            sted = re.findall(reg, txt_c)
            txt_ = ''.join(sted)
            txt_.split()
            st = txt_.split()[::2]
            dur = txt_.split()[1::2]
            ...
            for i in range(len(spkid)):
                if spkid[i] == cluster_id:
                    c.append(st[i])
                    seg_ed = float(st[i]) + float(dur[i])
                    d.append(seg_ed)
                    if i == len(spkid) - 1:
                        dur_st = - float(c[0]) + float(d[-1])
                        k.append(dur_st)
                        id.append(cluster_id)
            # print(id)
                else:
                    dur_st = float(st[i]) - float(c[0])
                    k.append(dur_st)
                    id.append(cluster_id)
                    if i == len(spkid) - 1:
                        seg_ed = float(st[i]) + float(dur[i])
                        dur_st = - float(d[-1]) + float(seg_ed)
                        k.append(dur_st)
                        id.append(cluster_id)
                        print(dur_st)
                        # print(id)
            ...
            for j in range(len(k)):
                if k[j] >= 0:
                    if k[j] < 0.5 and j != 0:
                        ...
                        k_new.append(k[j])
                        id_new.append(id[j])
                    elif j == len(k) - 1 and k_new[-1] != k[j]:
                        k_new.append(k[j])
                        id_new.append(id[j])
            flag = False
            num = 0
            ...
            for j in range(len(k_new)):
                total += 1000 * float(k_new[j])
                if flag:
                    break
                for i in range(len(res[0]['timestamp'])):
                    if res[0]['timestamp'][i][1] <= total and \
                            res[0]['timestamp'][min(i + 1, len(res[0]['timestamp']) - 1)][
                                0] > total:
                        # 修改
                        d_.append(res[0]['text'].rsplit()[num:i + 1])
                        num = i + 1
                    elif res[0]['timestamp'][i][0] <= total and res[0]['timestamp'][i][1] > total:
                        d_.append(res[0]['text'].rsplit()[num:i + 1])
                        num = i + 1
                    if total >= res[0]['timestamp'][-1][1]:
                        d_.append(res[0]['text'].rsplit()[num:])
                        flag = True

            txt_path = c_txt_out
            if len(id_new) != len(d_):
                id_new.pop(0)
            if len(id_new) == len(d_):
                with open(txt_path, 'w', encoding='utf-8') as f2:
                    for i in range(len(id_new)):
                        cnt += float(k[i])
                        tex = ' '.join(d_[i])  # ' '.join([str(item) for item in d[i][j]])
                        if len(tex) != 0:
                            if i == 0:
                                # tex = inference_pipline(input=tex)[0]['text']
                                f2.write(f'SPEAKER {id_new[i]} 0.0000-{cnt:.4f} text:{tex}\n')
                                print(f'SPEAKER {id_new[i]} 0.0000-{cnt:.4f} text:{tex}\n')
                            elif last == 0:
                                # tex = inference_pipline(input=tex)[0]['text']
                                f2.write(f'SPEAKER {id_new[i]} {cnt - float(k[i]):.4f}-{cnt:.4f} text:{tex}\n')
                                print(f'SPEAKER {id_new[i]} {cnt - float(k[i]):.4f}-{cnt:.4f} text:{tex}\n')
                            elif last != 0 and one:
                                # tex = inference_pipline(input=tex)[0]['text']
                                f2.write(f'SPEAKER {id_new[i]} 0.0000-{cnt:.4f} text:{tex}\n')
                                print(f'SPEAKER {id_new[i]} 0.0000-{cnt:.4f} text:{tex}\n')
                                ...
                                # tex = inference_pipline(input=tex)[0]['text']
                                f2.write(f'SPEAKER {id_new[i]} {last - float(k[i - 1]):.4f}-{cnt:.4f} text:{tex}\n')
                                print(f'SPEAKER {id_new[i]} {last - float(k[i - 1]):.4f}-{cnt:.4f} text:{tex}\n')
                                last = 0
                        else:
                            if i == 0:
                                one = True
