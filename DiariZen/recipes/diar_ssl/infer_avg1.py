...
def merge_closer(sample_rate,annotation,res, delta=1.0, max_len=60, min_len=10):
    name = annotation.uri
    speakers = annotation.labels()
    new_annotation = Annotation(uri=name)
    txt_annotation = new_annotation
    for spk in speakers:
        ...
        for seg in c_segments:
            if not stack or abs(stack[-1].end - seg.start) < delta:
                ...
                continue  # continue
            num=0
            for i in range(len(res[0]['timestamp'])):
                print(res[0]['timestamp'][i])
                print(f'stack{stack[0].start*sample_rate},{stack[0].start*sample_rate}')
                if res[0]['timestamp'][i][0] <= stack[0].start*sample_rate and res[0]['timestamp'][min(i + 1, len(res[0]['timestamp']) - 1)][
                0] > stack[0].start*sample_rate:
                    num=i
                if res[0]['timestamp'][i][1] <= stack[-1].end*sample_rate and res[0]['timestamp'][min(i + 1, len(res[0]['timestamp']) - 1)][
                                -1] > stack[0].end*sample_rate:
                    text=res[0]['text'].rsplit()[num:i + 1]
                    print(f'text={text}')
                    break
            ...
            if (stack[-1].end - stack[0].start) > max_len:
                ...
                for sub_seg in split_maxlen(stack, min_len):
                    ...
                    txt_annotation[sub_seg] = str(spk)+text[-1]
                stack = [seg]
            else:
                new_annotation[Segment(stack[0].start, stack[-1].end)] = spk
                txt_annotation[Segment(stack[0].start, stack[-1].end)] = str(spk)+text[-1]
                stack = [seg]

        if len(stack):
            print(f'new_ann={stack[0].start},stack[-1].end={stack[-1].end}')
            new_annotation[Segment(stack[0].start, stack[-1].end)] = spk
            txt_annotation[Segment(stack[0].start, stack[-1].end)] = str(spk)+text[-1]

    return new_annotation,txt_annotation
...
def diarize_session(res,
    ...
    new_annotation,txt_annotation = merge_closer(
        1000,new_annotation, res, delta=merge_closer_delta, max_len=max_length_merged, min_len=10
    )
    return new_annotation,txt_annotation
...
def read_uem(uem_file):
    ...
    return uem2sess
def rttm2txt(rttm_path,res,c_txt_out,start,end):
    inference_pipline = pipeline(
        task=Tasks.punctuation,
        model='/home/guyf/.cache/modelscope/hub/iic/punc_ct-transformer_cn-en-common-vocab471067-large',
    ...
    if a[-1][0] == 0:
        a[-1].remove(0)
    total, last, cnt = 0, 0, 0
    for k in a[-1]:
        total += 1000 * float(k)
        d = []
        for i in range(len(res[0]['timestamp'])):
            if res[0]['timestamp'][i][1] <= end and res[0]['timestamp'][min(i + 1, len(res[0]['timestamp']) - 1)][
                0] > end:
                # 修改
                d.append(res[0]['text'].rsplit()[num:i + 1])
                num = i + 1
            elif res[0]['timestamp'][i][0] <= end and res[0]['timestamp'][i][1] > end:
                d.append(res[0]['text'].rsplit()[num:i + 1])
                num = i + 1
        if end >= res[0]['timestamp'][-1][1]:
            d.append(res[0]['text'].rsplit()[num:])
        e.append(d)
    if len(b[-1]) != len(e):
        b[-1].pop(0)
    if len(b[-1]) == len(e):
        with open(c_txt_out, 'w') as f2:
            for i in range(len(b[-1])):
                cnt += float(a[-1][i])
                for j in range(len(e[i])):
                    tex = ' '.join([str(item) for item in e[i][j]])
                if len(tex) != 0:
                    if i == 0:
                        tex = inference_pipline(input=tex)[0]['text']
                        f2.write(f'SPEAKER {b[-1][i]} 0.0000-{cnt:.4f} text:{tex}\n')
                        print(f'SPEAKER {b[-1][i]} 0.0000-{cnt:.4f} text:{tex}\n')
                    elif last == 0:
                        tex = inference_pipline(input=tex)[0]['text']
                        f2.write(f'SPEAKER {b[-1][i]} {cnt - float(a[-1][i]):.4f}-{cnt:.4f} text:{tex}\n')
                        print(f'SPEAKER {b[-1][i]} {cnt - float(a[-1][i]):.4f}-{cnt:.4f} text:{tex}\n')
                    elif last != 0 and one:
                        tex = inference_pipline(input=tex)[0]['text']
                        f2.write(f'SPEAKER {b[-1][i]} 0.0000-{cnt:.4f} text:{tex}\n')
                        print(f'SPEAKER {b[-1][i]} 0.0000-{cnt:.4f} text:{tex}\n')
                        last = 0
                        one = False
                    elif last != 0 and not one:
                        tex = inference_pipline(input=tex)[0]['text']
                        f2.write(f'SPEAKER {b[-1][i]} {last - float(a[-1][i - 1]):.4f}-{cnt:.4f} text:{tex}\n')
                        print(f'SPEAKER {b[-1][i]} {last - float(a[-1][i - 1]):.4f}-{cnt:.4f} text:{tex}\n')
                        last = 0
                else:
                    if i == 0:
                        one = True
...
if __name__ == "__main__":
    ...
    model = AutoModel(
        model="/home/guyf/.cache/modelscope/hub/iic/speech_paraformer-large-vad-punc_asr_nat-zh-cn-16k-common-vocab8404-pytorch",
        model_revision="v2.0.4",
        vad_model="/home/guyf/.cache/modelscope/hub/iic/speech_fsmn_vad_zh-cn-16k-common-pytorch",
        vad_model_revision="v2.0.4",
        # punc_model="/home/guyf/.cache/modelscope/hub/iic/punc_ct-transformer_cn-en-common-vocab471067-large", punc_model_revision="v2.0.4",
    ...
    if args.uem_file:
        ...
        for sess in sess2audio.keys():
            ...
            if args.uem_file:
                ...
                c_uem = None
            res = model.generate(input=sess2audio[sess],
                                 batch_size_s=300)
            c_result,t_result = diarize_session(res,
                sess,
            ...
            c_txt_out = os.path.join(args.out_dir, sess + ".txt")
            with open(c_rttm_out, "w") as f:
                f.write(c_result.to_rttm())

            with open(c_txt_out, "w") as f:
                f.write(t_result.to_rttm())
