...
if __name__ == "__main__":
    ...
    path = '/home/guyf/DiariZen/recipes/faster-whisper-large-v2'
            ...
            with open(c_rttm_out, "w", encoding='utf-8') as f:
                for segment, spk, sent in final_result:
                    ...
                    if spk==speaker and (segment, spk, sent) != final_result[0]:
                        ...
                        sentence = re.sub(reg_sub, "", sentence)
                        print("%s 0 %s %.3f %.3f %s" % (sess, speaker, start[0], end[-1], sentence))
                        f.write("%s 0 %s %.3f %.3f %s" % (sess, speaker, start[0], end[-1], sentence))
                        f.write('\n')
                        speaker=spk
                        start=[]
                        end=[]
